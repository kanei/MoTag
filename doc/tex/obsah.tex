...
Ka¾dé slovo èeského jazyka má mnoho kategorií, do kterých mù¾e být zaøazeno -- od slovního druhu po slovesný vid. Ve strojovém zpracování jazyka lze pou¾ít nìkolika rùzných pøístupù k ulo¾ení tìchto informací. V na¹em pøípadì bude struktura znaèek odpovídat tzv. poziènímu systému\footnote[1]{Popis morfologických znaèek: http://ucnk.ff.cuni.cz/bonito/znacky.php} pou¾ívanému na pra¾ské Univerzitì Karlovì. Tento systém se skládá ze 16 pozic (viz. tabulka~\ref{tab:PopisZnacek}).
\begin{center}
	\begin{tabular}{|c|l|}
		\hline
		Pozice & Popis \\
...
\section{Data}
K trénování a následné evaluaci výsledkù máme k dispozici okolo 77 000 vìt. Data jsou získána z korpusu \emph{PDT 2.0}\footnote[1]{Stránky PDT 2.0: http://ufal.mff.cuni.cz/pdt2.0/} a ulo¾ena ve formátu dat \emph{CSTS}\footnote[2]{Definice: http://ufal.mff.cuni.cz/pdt2.0/doc/data-formats/csts/html/DTD-HOME.html}. Pomocí skriptù v jazyce \emph{Python} jsou tato data následnì pøevedena do formátu kompatibilního s pou¾itými programy.

\section{Existující øe¹ení pro znaèkování èeského jazyka}
Pro porovnání výsledkù této práce byl pou¾it program \emph{Morèe} vytvoøený na Ústavu formální a aplikované lingvistiky na Univerzitì Karlovì v Praze. K trénování je vyu¾íván skrytý markovský model s pou¾itím prùmìrovaného perceptronu. Vstupními daty je korpus \emph{PDT 2.0} s údaji ve formátu \emph{CSTS}. Podle dostupných údajù program dosahuje úspì¹nosti od 95 do 96 procent je tudí¾ nejpøesnìj¹ím znaèkovacím programem pro zpracování èeského jazyka. Z dùvodu pou¾ití stejných dat tedy nebude problém porovnat koneènou úspì¹nost námi pou¾ité implementace s úspì¹ností programu \emph{Morèe}. Dal¹í informace o konkurenèním øe¹ení a jeho funkcionalitì, které z velké èásti poslou¾ily i jako inspirace pøi této práci lze nalézt v~\cite{Votrubec}~a~\cite{Spoustova}.
...
Horní øada popisuje mno¾inu skrytých stavù($Q$) a plné èáry mo¾né pøechody mezi nimi s vyznaèenou pravdìpodobností($A$). Pøeru¹ované spojnice oznaèují pravdìpodobnosti vygenerování obrazcù z jednotlivých stavu($B$) a obrazce pod èarou (ètverec, kosoètverec, trojúhelník) jsou posloupností výstupních (pozorovaných) stavù($O$).

\paragraph{Vysvìtlení} 
Abychom získali v prvním kroku kruh, musíme aktuální stav, ve kterém se nacházíme, mít mo¾nost kruh vygenerovat -- v na¹em pøípadì jsou to stavy \emph{A} a \emph{B}. Zapí¹eme je tedy do prvního sloupeèku tabulky. Abychom jako druhý prvek posloupnosti získali kosoètverec, musíme být ve \emph{Stavu 2}, nebo» ten jako jediný poskytuje tento obrazec. Pøi prvním kroku jsme si vypsali \emph{A} a \emph{B}, ve \emph{B} v¹ak nemù¾eme setrvat a ten tudí¾ vypadává z mo¾ných posloupností a zùstává nám posloupnost (\emph{A}~$\rightarrow$~\emph{B}). Kdybychom takto pokraèovali a¾ do konce, získáme ètyøi mo¾nosti, zachycené v tabulce~\ref{tab:HMMPrechody}. Nejpravdìpodobnìj¹í z nich bychom byli schopni urèit pomocí ohodnocení jednotlivých pøechodù a výbìrem toho s nejvy¹¹í pravdìpodobností, k èemu¾ jsou vyu¾ity ní¾e popsané algoritmy, vyu¾ívající rùzných metod k optimalizaci tohoto procesu. 
...
\subsubsection*{Popis a definice}
Jak je popsáno v~\cite{Lafferty}, nech» $X$ je náhodnou promìnnou dat (v na¹em pøípadì vìt) urèených k oznaèkování a $Y$ jako náhodnou promìnnou korespondujících znaèek. V¹echny $Y_i$ z $Y$ nále¾í abecedì $\xi$, která tedy obsahuje mno¾inu v¹ech pou¾itých znaèek. Vytvoøíme pak model $P(X|Y)$ z párù pozorovaných hodnot a sekvencí znaèek. CRF pak mù¾eme vyjádøit následujícím odstavcem:
...
\label{sec:reg}
\subsubsection*{Základní informace}
Regularizace je pou¾ívána ve statistice a strojovém uèení k zamezení jevu známému jako \emph{overfitting} -- natrénovaní \uv{¹umu} do modelu, èím¾ se sice zvý¹í úspì¹nost na trénovacích datech, ale pøi reálném vyu¾ití úspì¹nost naopak klesá. Tomu se sna¾íme zabránit pomocí sni¾ování poètu rysù napojených na ka¾dý prvek modelu. Existuje nìkolik variant regularizací a jejich aplikací, v na¹em pøípadì jsou v¹ak vyu¾ity pouze \emph{L1-norm} a \emph{L2-norm}. Více o srovnání tìchto dvou v~\cite{Ng}.
...
Systém je tedy tvoøen pìti stránkami, kde dvì z nich slou¾í k vkládání dat (\emph{spu¹tìní nového testu}, \emph{dokonèení bì¾ícího testu}) a tøi k zobrazení ulo¾ených dat (\emph{zobrazení bì¾ících testù}, \emph{zobrazení dokonèených testù} a \emph{zobrazení náhledu na v¹echny testy a jejich rysy}). Ka¾dá tato èást je reprezentována samostatnou stránkou, pøístupnou v bloku systému drupal a tím pádem umístitelnou do kterékoliv èásti prezentace. Pro zpøehlednìní prezentace a zvìt¹ení pou¾itelného prostoru byl tento blok vlo¾en do patièky stránky.
...
\begin{figure}
	\begin{center}
		\begin{tabular}{|l|l|c|}
			...
			\hline
		\end{tabular}
		\captionof{table}{Charakteristika strojù pou¾itých pøi zpracování testù}
...
Na zaèátku øádku se nachází výsledná znaèka, následovaná jednotlivými rysy. V¹echny rysy jsou tak vytvoøeny pøed samotným spu¹tìním testovacího programu a v prùbìhu testování se ji¾ nemìní -- mohou být vypu¹tìny jen pøed samotným zaèátkem (kvùli limitu minimálního poètu výskytù), nebo pøi vykonávání regularizace.
...
\chapter{Testy}
\label{chap:testy}
Pøed zapoèatím øádného testování a analýzy výsledkù bylo potøeba se rozhodnout, s jakými parametry bude testování provádìno. V¹echny následující testy jsou provedeny v programu \emph{CRFSuite}, který, jak je zmínìno vý¹e, pro tuto práci pøiná¹í vhodnìj¹í prostøedí. V rámci této práce bude vìt¹ina testù provedena s regularizací L1 i L2 (viz. podkapitola~\ref{sec:reg}) a výsledky budou mezi sebou porovnány. Testy jsou pøevá¾nì provedádìny s vyu¾itím algoritmu L-BFGS (viz. podkapitola~\ref{sec:lbfgs}), v prùbìhu práce v¹ak byl pro nìkolik testù pou¾it algoritmus SGD (viz. podkapitola~\ref{sec:sgd}), který poskytl kvalitnìj¹í výsledky pøi ni¾¹í èasové nároènosti. Na provedení komplexního srovnání a otestování v¹ech vynechaných rysù v¹ak ji¾ nevy¹el èas. Do práce je tak pøidáno krátké srovnání výsledkù obou zmínìných algoritmù a závìry, zji¹tìné pro L-BFGS, budou pokusnì aplikovány i na SGD.
...
Následující testy se zabývají analýzou ideálního poètu vìt pro pou¾ití v dal¹ích èástech práce. Je nutno získat takovou hodnotu, která bude poskytovat jednak kvalitní, jednak èasovì akceptovatelné výsledky, které poskytnou mo¾nost zpracovat v¹echny èasovì nároèné testy na daném poètu strojù. 

\subsection{Konstantní poèet iterací a regularizace L1}

\subsubsection*{Pøíprava}
Nejdøíve bylo tøeba rozhodnout, jaký poèet iterací bude vhodný pro proces testování. Po nìkolika pokusných testech se jako nejlep¹í ukázalo èíslo 50. Okolo této hodnoty program ji¾ zaèal pùlit kroky algoritmu pro vy¹¹í pøesnost a tím se exponencionálnì zvy¹ovala nároènost ka¾dého dal¹ího kroku aproximace parametru. K dokonèení 60. iterace proto èas vzrostl pøibli¾nì 3krát, zatímco úspì¹nost se zvý¹ila pouze minimálnì.
...
U L2 je v porovnání s regularizací L1 úspì¹nost (tabulka~\ref{tab:L1L2KonstIterEval}) úspì¹nost v rámci jednotlivých slov vy¹¹í o nìkolik procent a u celých vìt je rozdíl dokonce nìkolik desítek procent (nepoèítáme-li výsledky zpùsobené anomálií vyskytující se pøi nìkterých výsledcích u L1). Toto zlep¹ení je v¹ak vykoupeno skuteèností, ¾e velikost modelu (tabulka~\ref{tab:L1L2KonstIterData}) je v prùmìru 4krát vy¹¹í a doba testování ze zvý¹ila pøibli¾nì o polovinu. Je tedy otázkou, zda je pro nás tato skuteènost akceptovatelná a zda nám stojí za zvý¹ení úspì¹nosti o nìkolik procent, nebo zda dáme pøednost malému modelu a rychlej¹ímu zpracování -- to u¾ v¹ak zále¾í na konkrétním vyu¾ití programu. Pøekvapivì se v¹ak nijak nezvý¹ila doba trénování.
...
Naopak nejvíce pøínosné se zdá být pravidlo popisující znaèku nejbli¾¹ího slovesa. Ostatní prvky, které se ukázaly být dùle¾ité v druhém testu, vyu¾ívajícím regularizaci L2, v tomto pøípadì jsou ovlivnìny chybou a proto je tì¾ké rozhodnout, zda mají pro testování stejný pøínos.
...
\subsection{Regularizace L1}
Pro tyto testy byly nakonec pou¾ity pouze dva kroky: bez tøí a ¹esti nejménì vhodných pravidel. Takto malý poèet testù byl zpùsoben anomáliemi pøi regularizaci L1 a nepøesvìdèívými výsledky po jejich provedení. Pøi vynechání prvních tøech pravidel se úspì¹nost zvý¹ila o 28 setin procenta, pøi dal¹ích tøech se ale ji¾ sní¾ila pod úroveò úspì¹nosti s vynecháním pouze jediného prvku. Zøejmì by bylo mo¾né je¹tì provést nìkolik dal¹ích testù za pøidání dal¹ích prvkù k prvnímu kroku, z èasových dùvodù v¹ak byla vypoèetní síla pøesunuta na testy s vy¹¹í hodnotou. Jediná pøínosná èást pøi tomto nastavení by tedy mohla být velikost modelu, která se nakonec pohybuje okolo 16MB a je tedy mnohem ni¾¹í, ne¾ u L2. Více podrobností v tabulkách~\ref{tab:KrokyL1}~a~\ref{tab:NevhodneTestL1}.
...
\subsection{Regularizace L2}
V této èásti budou popsány výsledky s pou¾itím regularizace L2 v algoritmech L-BFGS a SGD a výsledky budou vzájemnì porovnány. Jako referenèní hodnota nebude slou¾it nejlep¹í výsledek pøi vynechání jednotlivých pravidel, jak tomu bylo v pøedchozí kapitole, ale hodnota pøi vyu¾ití v¹ech pravidel. Dùvodem k tomu je absence dat s jednotlivými vypu¹tìnými pravidly pro algoritmus SGD a tím pádem nemo¾nost vyu¾ití daných hodnot ke vzájemnému srovnáni. Jednotlivé kroky s vypu¹tìnými pravidly jsou popsány v tabulce~\ref{tab:KrokyL2} - Nejprve jsou vypu¹tìna tøi nejménì vhodná pravidla a v ka¾dém dal¹ím kroku jsou k nim pøidána dal¹í dvì. 
...
Výsledky za pou¾ití algoritmu L-BFGS jsou zachyceny v tabulce~\ref{tab:NevhodneTestL2BFGS}. Mù¾eme vykpozorovat, ¾e s vìt¹ím poètem vynechaných pravidel se zvy¹uje poèet uskuteènìných iterací, potøebných k dosa¾ení rozdílu mezi iteracemi o hodnotì $\Delta$. Celkový potøebný èas se v¹ak viditelnì nezvý¹il. Poèet rysù se pøi v kroku 4 sní¾il a¾ na dvì tøetiny velikosti originální mno¾iny a tím pádem se sní¾ila i velikost modelu z pùvodních 77MB na 45MB.

Zatímco bez vynechání jakéhokoliv rysu je úspì¹nost urèení jednotlivých slov rovna $92,17\%$, v prvním kroku se zvý¹ila na $93.3\%$. Pøi vynechání dal¹ích dvou prvkù se ov¹em sní¾ila o $0.3\%$, z èeho¾ vyplývá, ¾e alespoò jedno z pravidel U23 a U52 je vhodné zachovat. Pøi vynechání dal¹ích dvou se úspì¹nost opìt zvý¹ila a je dokonce o $0.01\%$ vy¹¹í, ne¾ v kroku 1. Pravidla U42 a U52 (nebo jedno z nich) tedy nejsou pro testování vhodná a je vhodnìj¹í je do mno¾iny pravidel nezahrnout. Pøi kroku 4 se opìt dostáváme k hodnotì $93\%$ a tím pádem na úroveò kroku 2, tato pravidla je tím pádem opìt výhodnìj¹í zachovat.

Pro dal¹í zlep¹ení výsledkù by bylo vhodné vytvoøit více skupin vypu¹tìných pravidel a hloubìji zanalyzovat dùsledky jejich vynechání na celkovou úspì¹nost. 
...
\subsubsection*{SGD}
Z tabulky~\ref{tab:NevhodneTestL2SGD} je vidìt, ¾e u algoritmu SGD se pøi vynechávání rysù poèet iterací nemìní, stejnì jako se nezvy¹uje ani doba trénování. Poèet rysù a velikost modelu je pak témìø identická jako v pøípadì algoritmu L-BFGS. Co se týèe úspì¹nosti urèení jednotlivých slov, je situacie podobná jako v pøedchozím testu. Jediným rozdílem je, ¾e algoritmus SGD je zøejmì schopen lépe vynechat nevhodná pravidla a nejvy¹¹í úspì¹nosti dosahuje pøi vyu¾ití v¹ech pravidel. Pøi kroku 1 se v¹ak úspì¹nost sní¾í pouze nepatrnì a to o $0.01\%$. U následujících krokù se opakuje stejný prùbìh jako u L-BFGS, kdy u kroku 2 se sní¾í úspì¹nost, aby se pøi kroku 3 opìt o kousek zvý¹ila a v kroku 4 klesla na úroveò kroku 2.
...
\subsubsection*{Srovnani L-BFGS a SGD}
Koneèné srovnání je zachyceno v tabulce ~\ref{tab:tab:KonecneSrovnani}. Jak jde vidìt, nejlep¹í výsledky podává algoritmus SGD ji¾ pøi vyu¾ití v¹ech pravidel rysù a L-BFGS by se mu mohl teoreticky pøiblí¾it pouze po dùkladné analýze v¹ech pou¾itých pravidel a vynechání v¹ech nevhodných. Zatímco u L-BFGS je krok 4 pøibli¾nì stejnì úspì¹ný jako krok 2, u SGD jsou kroky 2 a¾ 4 na podobné úrovni a tudí¾ pøibli¾nì o $0.3\%$ ménì úspì¹né, ne¾ v pøípadì vyu¾ití v¹ech pravidel, nebo pøi kroku 1.
...
\begin{table}[htbp]
	\begin{center}
		...
		\label{tab:KonecneSrovnani}
	\end{center}
\end{table}

\section{Srovnání s programem Morèe}
Nejlep¹í získanou úspì¹ností je $94.51\%$ pøi vyu¾ití algoritmu SGD, 50 000 vstupních vìtách a $\Delta = 0.1$. Z toho vyplývá, ¾e je stále mo¾no pøidat do testovací mno¾iny dal¹ích 27 000 vìt a zvý¹it pøesnost o nìkolik øádù. Celý proces testování by pak nemìl pøesáhnout nìkolik týdnù a výsledná hodnota úspì¹nosti by se pak mìla je¹tì zvý¹it. Tyto hodnoty v¹ak byly získány se zkrácenou znaèkou a nemù¾eme je tedy pøímo porovnávat s úspì¹ností programu Morèe. K tomu byl uskuteènìn test s vyu¾itím algoritmu SGD, 30 000 vìtami a $\delta = 0.1$. Jeho koneèná úspì¹nost je $92.72\%$, co¾ je oproti $93.96\%$ pøi vyu¾ití èásteèné znaèky sní¾ení o více ne¾ jedno procento. Pokud bychom testy spustili se v¹emi dostupnými daty a pøi vy¹¹í pøesnosti, nejspí¹e bychom se stále dr¾eli daleko od hodnoty $96\%$, které by mìl dosahovat program Morèe. 

\chapter{Závìr}
Pøi zpracování této práce byla vytvoøena mno¾ina pravidel pro generování rysù a s pou¾itím algoritmu Conditional Random Fields byla otestována na mno¾inì vstupních dat. Byly otestovány regularizace L1 a L2, kde první zmínìná poskytuje men¹í výsledný model, ale také ni¾¹í úspì¹nost. Za vyu¾ití algoritmu L-BFGS pak byly zanalyzována v¹echna pravidla a ta nejhor¹í pak byla postupnì vynechávána za úèelem zaji¹tìní co nejlep¹í úspì¹nosti, která se nakonec dostala a¾ na hodnotu $93.31\%$ pøi 30 000 vstupních vìtách. Výsledky zji¹tìné pøi vynechání jednotlivých mno¾in pravidel také naznaèují, ¾e pøi jiné kombinaci pou¾itých a vynechaných pravidel by se mohla koneèná úspì¹nost dále zvy¹ovat. 

Skupiny pravidel pro generování rysù, nevhodných pro L-BFGS, byly postupnì vynechávány i pøi vyu¾ití algoritmu SGD, jeho¾ úspì¹nost se tím v¹ak nenavy¹ovala, ale naopak mírnì sní¾ila. Celkovì v¹ak SGD byl o nìkolik procent úspì¹nìjsí a pøi vyu¾ití v¹ech pravidel poskytoval vy¹¹í výsledky, ne¾ L-BFGS po vyu¾ití v¹ech získaných poznatkù a vynechání nevhodných pravidel. Nejlep¹í získanou hodnotou je pak $94.51\%$ pøi 50 000 vstupních vìtách. Dal¹ího zvý¹ení úspì¹nosti bychom mohli dosáhnout pøetestováním tohoto algoritmu stejným zpùsobem, jako L-BFGS (vynecháním jednotlivých pravidel a porovnáním s referenèní hodnotou), algoritmu by to v¹ak zøejmì úspì¹nost zvý¹ilo pouze minimálnì.

Jeliko¾ byly témìø v¹echny testy provedeny pouze za pou¾ití zkrácené znaèky, nabízí se otestování tìchto pravidel i s vyu¾itím plné znaèky a tím pádem dal¹ích mo¾ností pro generování rysù pomocí informací, které byly v této práci pomíjeny. Také by bylo vhodné vyu¾ít v¹echna dostupná data, pøípadnì dal¹í korpusy, a zvý¹it pøesnost trénování pro detailnìj¹í informace.

Dal¹ím mo¾ností pro zvý¹ení úspì¹nosti je vytvoøení vlastní implementace, která by byla schopna dynamicky generovat rysy za bìhu programu a nevyu¾ívat pouze ty, které byly vygenerovány pøed zapoèetím zpracování --byla by tak mnohem vhodnìj¹í pro zpracování èeského jazyka. Tento pøístup by v¹ak byl mnohem nároènìj¹í na výpoèetní výkon, nebo» by bylo nutno místo èíselných konstant vyu¾ívat øetìzce, obsahujícího v¹echny pozice znaèky.
